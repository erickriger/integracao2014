...
public class RegistroActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		btnRegistrarGCM = (Button) findViewById(R.id.btnRegistrarGCM);
			...
			public void onClick(View arg0) {
				if (TextUtils.isEmpty(regId)) {
					...
					Toast.makeText(getApplicationContext(),
							"Já registrado no GCM Server!",
							Toast.LENGTH_LONG).show();
				}
			}
		});

		btnCompartilharId = (Button) findViewById(R.id.btnCompartilharRegId);
		btnCompartilharId.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View arg0) {
				if (TextUtils.isEmpty(regId)) {
					Toast.makeText(getApplicationContext(), "RegId está vazio!",
							Toast.LENGTH_LONG).show();
				} else {
					Intent i = new Intent(getApplicationContext(), MainActivity.class);
					i.putExtra("regId", regId);
					Log.d("RegistroActivity",
							"onClick no Compartilhar: antes de iniciar a main activity.");
					startActivity(i);
					finish();
					Log.d("RegistroActivity", "onClick no Compartilhar: depois de terminar.");
				}
			}
		});
	}
	...
	public String registrarGCM() {
		...
		if (TextUtils.isEmpty(regId)) {
			...
			Log.d("RegistroActivity",
					"registrarGCM - registrado no GCM Server com sucesso - regId: "
							+ regId);
		} else {
			Toast.makeText(getApplicationContext(),
		...
		new AsyncTask<Void, Void, String>() {
			...
			protected String doInBackground(Void... params) {
				...
				try {
					...
					Log.d("RegistroActivity", "Erro: " + msg);
				}
				Log.d("RegistroActivity", "AsyncTask completada: " + msg);
				return msg;
			}
			...
			protected void onPostExecute(String msg) {
				Toast.makeText(getApplicationContext(),
						"Registrado no GCM Server." + msg, Toast.LENGTH_LONG)
						.show();
			}
